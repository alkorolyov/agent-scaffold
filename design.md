...
│   ├── records/                         # Project ledger — exp / feat / bug / decision (per-record files)
...
#### tools/exp.py — the experiment CLI (the *write* path)

The AI should never hand-edit per-exp markdown frontmatter directly. Every status / metric / takeaway / lineage update goes through this CLI. Output is short and structured — token-cheap. The CLI enforces schema invariants so drift becomes impossible.

```bash
# Create new exp file with auto-incremented number, frontmatter scaffolded
$ tools/exp.py new "lgbm scaffold split"
Created docs/experiments/exp_42_lgbm_scaffold_split.md (status=planned)

# Update frontmatter — writes ONLY frontmatter, body untouched
$ tools/exp.py update 42 --status=done --outcome=refuted \
    --takeaway="Scaffold splits drop AUC by 0.04; random splits leak."
Updated exp_42 (status: planned → done). Re-rendered INDEX.md.

$ tools/exp.py update 84 --status=done --outcome=improved \
...
$ tools/exp.py update 13 --status=done --outcome=infra \
    --takeaway="vast.ai 5090 instances stable for 12hr GAT runs."
Updated exp_13.

# Show one experiment's frontmatter (cheap lookup, no body load)
$ tools/exp.py show 84
...
# Filter (replaces grep-the-monolith and replaces a vector index for ID-based lookups)
$ tools/exp.py list --tag=gnn --status=done --outcome=improved
 84  done  improved  auc=0.938  SHAP top-100 subgraph...
114  done  improved  auc=0.941  MI top-100 + ESM2 wins...

# Walk the lineage DAG
$ tools/exp.py lineage 114
...
# Append a finding linked to an exp (one line in findings.md, no file read)
$ tools/exp.py finding 84 "SHAP feature selection beats MI on subgraph variants."
Appended to docs/findings.md.
```

**CLI rules** (refuses writes that violate them):

- `--status=done` requires `--outcome` and `--takeaway`.
- `--outcome=improved | parity | regression` requires `--track` and `--metric`.
- `--track=X` without `--metric` is rejected — you can't compete on a leaderboard without a number.
- `--parent=N` rejected if exp N doesn't exist or is `status: superseded` (would create dangling lineage).
...
- `INDEX.md` mtime older than any `exp_*.md` mtime (render is stale)
...
|-------|-------|----------|
| `/log-exp <NN>` | `tools/rec.py update` | Walk through frontmatter fields, compute lineage, update record, run `render_index.py` |
